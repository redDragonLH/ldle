...
# 总结
* **Set** 总是比线性查找更快，但是注意内存使用量,并且Set 对象生成也比较快 [1925. 统计平方和三元组的数目](leetcode\algorithm\1925-count-square-sum-triples.js)
* 根据 [1925. 统计平方和三元组的数目](leetcode\algorithm\1925-count-square-sum-triples.js) 的表现,底层优化过的方法类似 `sqrt` 在使用时总是比自定义方法或者未优化过的方法快，甚至快很多
   * 就算是使用了Set 方法也没办法和使用了sqrt的执行重复平方的官方题解算法媲美
   * 使用C/C++ 方法应该是一个JS优化点

### 切比雪夫距离
两个点（比如在平面上有(x1, y1)和(x2, y2)）之间，切比雪夫距离不关心你横竖走了多少“冤枉路”，它只关心你在横坐标方向差了多少，和在纵坐标方向差了多少，然后取这两个差值中最大的那个。
